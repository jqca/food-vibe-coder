...
const AIChat: React.FC<Props> = ({ onGenerate, onReset, isGenerating, history }) => {
  ...
  const handleReset = () => { setPrompt(''); setSelectedId(null); onReset(); };
  const handleUseCaseClick = (id: string, ucPrompt: string) => { setSelectedId(id); setPrompt(ucPrompt); };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ flex: '1', overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {history.length > 0 && (
          <button onClick={handleReset} disabled={isGenerating} className="back-to-list-btn">
        ...
        {history.length === 0 && (
          <div style={{ textAlign: 'center', opacity: 0.7, marginTop: '20px' }}>
            ...
            <p style={{ marginBottom: '16px' }}>食品・飲料 Quantum Copilot へようこそ。<br/>以下のユースケースから実装を選択してください：</p>
          </div>
        )}
        ...
        {history.map((msg, idx) => (
          <div key={idx} style={{ alignSelf: msg.role === 'user' ? 'flex-end' : 'flex-start', background: msg.role === 'user' ? 'rgba(0, 255, 157, 0.1)' : 'rgba(255,255,255,0.05)', border: `1px solid ${msg.role === 'user' ? 'var(--quantum-green)' : 'var(--border-color)'}`, padding: '12px', borderRadius: '12px', maxWidth: '90%', fontSize: '0.85rem', lineHeight: '1.5' }}>{msg.content}</div>
        ))}
        {isGenerating && (
      ...
      <div style={{ padding: '16px', borderTop: '1px solid var(--border-color)', background: 'var(--bg-panel)' }}>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <textarea value={prompt} onChange={(e) => { setPrompt(e.target.value); setSelectedId(null); }} placeholder="ユースケースを選択するか、直接プロンプトを入力..." rows={3} disabled={isGenerating} style={{ resize: 'none', width: '100%', padding: '12px', background: 'rgba(0,0,0,0.3)', border: `1px solid ${prompt ? 'var(--quantum-blue)' : 'var(--border-color)'}`, borderRadius: '8px', color: '#fff', fontSize: '0.85rem', lineHeight: '1.5', outline: 'none' }} />
          <button type="submit" className="btn-primary" disabled={isGenerating || !prompt.trim()} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
            {isGenerating ? <span className="anim-pulse">生成中...</span> : <><TerminalSquare size={16} /> Vibe Coding</>}
          </button>
        </form>
      </div>
    </div>
