        ...
        <pre style={{ margin: 0, whiteSpace: 'pre-wrap', wordBreak: 'break-all', filter: 'blur(2.5px)' }}>
          <code>
            {displayCode.split('\n').map((line, idx) => {
              ...
              const t = line.trim();
              if (t.startsWith('#')) color = 'var(--text-muted)';
              else if (t.startsWith('import') || t.startsWith('from') || t.startsWith('class ') || t.startsWith('def ')) color = '#ff7b72';
              else if (t.startsWith('def ') || t.includes('def ')) color = '#79c0ff';
